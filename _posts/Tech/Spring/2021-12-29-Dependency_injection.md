...
뮤지컬 주연은 배우를 선택하지 않는다. 배우를 선택하는 건 뮤지컬 감독이 한다. 마찬가지다. Mouse 라는 주연, 즉 인터페이스는 CableMouse 라는 배우, 즉 객체를 선택하지 않는다. **우리는 뮤지컬 감독의 역할이 필요하다.** 즉, 감독의 역할을 맡는  config 파일을 생성할 수도 있고, 혹은 Config 라는 클래스를 만들어서 사용할 수도 있다.
