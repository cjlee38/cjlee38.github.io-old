...
즉, 이렇게 생긴 것을 **""** 이라고 하고, Hash테이블을 통해 Key-Value를 관리하게 된다.  
...
뭐 대충 10이라는 숫자가 나왔다고 가정해보고 넘어가자.
...
if ((p = tab[i = (n - 1) & hash]) == null)
    tab[i] = newNode(hash, key, value, null);
```
다음으로, table의 ((n-1) & hash) 위치에 값이 null 이라면,  
즉, 자리가 비어있다면,    
