...
그런데, 이렇게만 하면 정보가 부족하다. 가령, 5번 Node의 경우, 2번 Node로부터 패배했기 때문에 5위라는 순위를 얻을 수 있는데, 위 그래프만으로는 이를 설명할 수 없다.따라서, **방향을 뒤집어서도**, 특정 Node를 가리킬 수 있는 Node들의 리스트 또한 만들어 줘야 한다.
