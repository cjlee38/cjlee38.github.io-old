...
# 0. 들어가며
: [이전 포스팅]() 에서, 웹 크롤링이 무엇인지 간략하게나마 이해했다. 이전 포스팅을 읽어보지 않았다면, 꼭 읽고오자.
...
그렇다면, 화면과 같이 Application에 대한 정보를 입력할 수 있다.
...
<center> 다 입력하고나면 이런식으로 된다. </center>

등록을 하고나면, 본인의 Client ID와 Client Secret을 확인할 수 있다. 이를 어딘가에 메모해두자. 여기까지 하면, 준비는 끝났다.
...
이는 아직 json으로 해석되지 않은 단순 문자열이며, 이를 json으로 해석해줘야한다.
